...
public class UserService implements HealthCheck {
    ...
    public User createUser(User user) throws URISyntaxException, Exception { // [validate that this works]
        ...
        if (findClient(clientRestClient.findAllClients(), user.identificationDocumentNumber,
                user.identificationDocumentType) == null) {

            throw new NotFoundException(
                    "The identification document number " + user.identificationDocumentNumber + " doesn't exists");
        }
        ...
        // validate that the debit card is not registered yet [needs to implement
        // calling the card microservice]
    ...
    @Transactional
    public User updateUser(String id, User user) { // [evaluate if this method is necessary because the card data should
                                                   // not change]
     ...
     * 
     * @Transactional
     * public User updateUser(User user) { // [evaluate if this method is necessary
     * because the card data should not change]
     * 
     * if (userRepository.findById((new ObjectId(user.userId.toString()))) != null)
     * {
     * 
     * User userToUpdate = userRepository.findById(new
     ...
     * userToUpdate.identificationDocumentNumber =
    ...
    public ClientRestClient findClient(List<ClientRestClient> clientList, String documentIdentity,
            String documentIdentityType) {

        logger.info("[RRM] findClient");

        ClientRestClient client = clientList.stream().filter(x -> x.documentIdentity.equalsIgnoreCase(documentIdentity))
    ...
    public AccountRestClient findAccount(List<AccountRestClient> accountList, Integer clientId) {
        ...
        AccountRestClient account = accountList.stream().filter(x -> x.clientId == clientId && x.isMain.equals(true))
                .collect(Collectors.toList()).get(0);
    ...
    public DebitCardRestClient findDebitCard(List<DebitCardRestClient> debitCardList, Integer accountId, User user) {
        ...
        DebitCardRestClient debitCard = debitCardList.stream()
                .filter(x -> x.accountId == accountId && x.cardNumber.equalsIgnoreCase(user.debitCardNumber)
                        && x.expirationDate.equalsIgnoreCase(user.debitCardDueDate)
                        && x.cardValidationCode.equalsIgnoreCase(user.debitCardValidationCode))
                .collect(Collectors.toList()).get(0);
    ...
    /*
     * public boolean validateDebitCard(List<DebitCard> debitCardList, String
     * cardNumber, String expirationDate, String cardValidationCode) {
     * 
     * DebitCard debitCard = debitCardList.stream().filter(x ->
     * x.documentIdentity.equalsIgnoreCase(documentIdentity)).collect(Collectors.
     * toList()).get(0);
     * 
     * if(client != null) {
     * if(client.documentIdentityType.equalsIgnoreCase(documentIdentityType)){
